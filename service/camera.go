...
func generateUrl(camera *model.Camera) {
	...
	case model.ADJ:
		camera.Url = fmt.Sprintf("http://%s:%d/videostream.cgi?user=%s&pwd=%s", camera.Host, camera.Port, camera.Username, camera.Password)
	case model.Microcam:
		camera.Url = fmt.Sprintf("http://%s:%d/media?action=snapshot&user=%s&pwd=%s", camera.Host, camera.Port, camera.Username, camera.Password)
	case model.SV3C:
		camera.Url = fmt.Sprintf("http://%s:%d/sv3c/%s/tmpfs/snap.jpg", camera.Host, camera.Port, camera.Name)
	case model.IFrame:
		camera.Url = fmt.Sprintf("http://%s:%d/%s", camera.Host, camera.Port, camera.Username)
	}
}
